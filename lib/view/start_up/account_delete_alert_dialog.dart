...
class AccountDeleteAlertDialog extends StatefulWidget {
  ...
  @override
  State<AccountDeleteAlertDialog> createState() =>
      _AccountDeleteAlertDialogState();
}
...
class _AccountDeleteAlertDialogState extends State<AccountDeleteAlertDialog> {
  ...
  Future onDeletedAccount() async {
    ...
    final myAccount =
        await AccountFirestore.getMyAccount(currentFirebaseUser.uid);
    if (myAccount == null) {
      return deleteErrorHandling('アカウント情報の取得に失敗しました');
    }
    final deleteAccountResult =
        await AccountFirestore.deleteAccount(currentFirebaseUser.uid);
    if (!deleteAccountResult) {
      return deleteErrorHandling('アカウントの削除に失敗しました');
    }
    final reAuthResult = await AuthenticationFirestore.reAuthenticate(
        currentFirebaseUser, passwordController.text);
    ...
    final deleteAuthResult =
        await AuthenticationFirestore.deleteAuth(currentFirebaseUser);
    if (!deleteAuthResult) {
      // 先ほど削除したアカウントを作り直す ( Authentication と Firestore の垣根を越えたトランザクション処理ができないため )
      await AccountFirestore.setNewAccount(myAccount);
      return deleteErrorHandling('アカウントの削除に失敗しました');
